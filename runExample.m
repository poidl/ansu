...
[s,ct,p,lon,lat]=get_input_jackett96_no_land_mask(path_to_gammanc); % read the original Jackett and McDougall (1997) data set (no land mask)
...
lon_=squeeze(lon(1,:,:));
lat_=squeeze(lat(1,:,:));
[ibb,ilat,ilon]=backbone_index(lon_,lat_); % open/modify backbone_index() to change lateral location of bb

pbot=p(sum(~isnan(s(:,ibb))),ibb);
pbb=linspace(0,pbot,102);
pbb=pbb(2:end-1);
% get gamma_n values at clamping points. Note that gvals is computed below, but not used after that. 
% This is just one way to obtain a 'label' for the surface. 
...
for kk=1:ns
    ...
    sms(kk)=error_surface(sns,ctns,pns,s,ct,p,lon,lat,'slope_difference'); % compute the error of the surface
...
omega_user_input;
save([datapath,'/himmat_omega_3d.mat'],'sns3d','ctns3d','pns3d')
